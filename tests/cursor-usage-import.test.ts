...
describe("matchCursorUsageRows", () => {
  test("pairs equal-count agent calls by duration order", () => {
    ...
    expect(matches[0]?.costEstimated).toBe(false)
    expect(matches[0]?.costAvailable).toBe(false)
...
describe("applyCursorUsageImport", () => {
  test("writes import overlay and session telemetry for a run", async () => {
    ...
    expect(sessionTelemetry.sessions[0]?.calls[0]?.usage?.costEstimated).toBe(false)
    expect(sessionTelemetry.sessions[0]?.calls[0]?.usage?.costAvailable).toBe(false)
