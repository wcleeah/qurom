...
- Return a single complete HTML file with all CSS inline. External script tags on trusted CDNs (cdnjs, jsdelivr, unpkg) are allowed and encouraged for libraries; they save output tokens and will not be truncated. Custom application JS should be inline. Include HTML comment blocks above each external script tag documenting name, version, source URL, and license.
