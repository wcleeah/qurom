...
> **Status: completed (archived).** The OpenTUI shell shipped under `src/tui/`. The web dashboard (`bun run dev`) is now the primary operator UI; this plan documents the earlier TUI-first rewrite. Paths under `reference/` were local git submodule checkouts and are no longer bundled in this repo.
