...
- Raise a finding when alternatives, exceptions, or supporting detail obscure the central path through the material.
- Raise a finding when the same claim, boundary, example, or decision is reproduced across multiple sections without gaining a new function.
- Treat source annotations as a clarity problem only when they become a second exposition rather than necessary attribution.
