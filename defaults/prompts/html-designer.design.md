...
- Mobile baseline: content must remain readable on narrow screens (e.g. wide code/tables may scroll horizontally). Deeper ergonomics belong later. Overlays (`<dialog>`, `[role="dialog"]`, lightboxes, detail panels) must stay fully inside a ~390px viewport: titles, comparison figures, labels, and source lines must not clip. Do not combine native dialog centering (`margin: auto`) with `left: 50%` / `translate(-50%)`. Size overlays with `width: min(<desktop-width>, calc(100vw - 2rem))`, `max-height: calc(100dvh - 2rem)`, `box-sizing: border-box`, equal horizontal padding, and internal `overflow: auto`. Never use `width: 100vw` or negative horizontal margins inside an overlay. Stack two-column "A vs B" rows on narrow screens.
