...
{standingContext}
Unresolved findings are in findings.json when that file is attached, and via the `get_unresolved_findings` tool on this session. Treat those as the source of truth. If this conversation was compacted and the findings are no longer in the prompt, re-read findings.json or call `get_unresolved_findings` rather than guessing. The current article is the working markdown file from the previous writing step. Edit that file in place. Do not rewrite the whole article, and do not create a new draft file.
