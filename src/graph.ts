...
export type DesignPhase = "drafting" | "enhancing" | "reading" | "reviewing" | "finalizing"
...
export function auditPrompt(
  ...
  agent: string,
  state: ResearchState,
  outputFileOrPreviousUnresolved?: string | AggregatedFinding[],
  previousUnresolvedInput?: AggregatedFinding[],
) {
...
export async function discoverReaderPrompt(
  ...
  const transcriptText = formatReaderTranscriptForPrompt(transcript)

  let turnResult: ReaderInterviewTurn | undefined
  try {
    for (let attempt = 1; attempt <= 2; attempt++) {
      const prompt = renderReaderInterviewPrompt({
        config,
        promptBundle,
        state,
        transcriptText,
      ...
      telemetry?.debugLog?.write("reader.interview_duplicate_question_retry", { turn, newQuestions: turnResult.newQuestions, attempt })
    }
  } catch (error) {
    await disposeReaderInterviewerSession(state.requestId)
    throw error
...
export async function draftFullDraft(
  config: RuntimeConfig,
  runtime: AgentRuntime,
  promptBundle: PromptBundle,
  state: ResearchState,
...
async function getOrCreateDrafterWritingHandle(input: {
  runtime: AgentRuntime
  state: ResearchState
  ...
  for (const agent of auditors) {
    auditPromises.push(
      (async () => {
        ...
        await ensureJsonArtifact(outputFile, response.structured, `audit ${agent}`)
...
async function runTargetedRebuttals(
  ...
  async function runAgentRebuttal(
    ...
    try {
      ...
      await ensureJsonArtifact(outputFile, response.structured, `rebuttal response ${agent}`)
...
export async function resolveDesignMarkdownPath(input: {
  ...
  const workingPath = draftWorkingPath(input.outputPath)
  if (await isNonEmptyTextFile(workingPath)) return workingPath
...
export function createGraph(
  ...
  return new StateGraph(researchStateObjectSchema, {
    ...
    .addNode("htmlReview", async (state) =>
      withNodeTelemetry("htmlReview", state, () =>
        htmlReviewNode(config, runtime, promptBundle, state, graphTelemetry, observer),
      ),
    )
    ...
    .addEdge("readingExperienceEnhance", "htmlReview")
    .addEdge("htmlReview", "finalizeDesign")
